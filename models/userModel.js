...
async function createUser({ firstname, lastname, email, passwordHash }, client = null) {
  const q = `
    INSERT INTO kasikos_owner (firstname, lastname, email, password_hash)
    VALUES ($1, $2, $3, $4)
...
async function findByEmail(email) {
  const res = await pool.query('SELECT * FROM kasikos_owner WHERE email = $1', [email]);
  return res.rows[0];
}

async function findById(id) {
  const res = await pool.query(
    'SELECT id, firstname, lastname, email, created_at FROM kasikos_owner WHERE id = $1',
    [id]
  );
  return res.rows[0];
}
